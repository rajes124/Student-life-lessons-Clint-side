...
import { 
  Lock, Heart, Bookmark, Share2, Flag, Calendar, Clock, User, Eye, AlertCircle 
} from "lucide-react";
...
const LessonDetails = () => {
  ...
  useEffect(() => {
    const fetchLesson = async () => {
      try {
        ...
        // Random views count (replace with real backend count in production)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Update saved count (assuming backend returns updated lesson or use optimistic update)
      setLesson(prev => ({
        ...prev,
        savedBy: saved 
      ...
      toast.success(saved ? "Removed from favorites." : "Saved to favorites!", {
        icon: <Bookmark className="w-6 h-6 text-blue-500 fill-blue-500" />,
        duration: 3000,
      });
    } catch (error) {
  ...
  const handleReport = () => {
    ...
    if (reason && reason.trim()) {
      ...
      // api.post(`/lessons/report/${id}`, { reason });
      toast.success("Lesson reported successfully. Thank you for helping keep the community safe.", {
