...
const MyProfile = () => {
  ...
  const [isFocused, setIsFocused] = useState(null); // Input focus state tracking
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-8 lg:p-12 bg-white/80 backdrop-blur-md rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.1)] mt-10 border border-white/20">
      ...
      <div className="relative text-center mb-16">
        <h2 className="text-4xl md:text-6xl font-black text-indigo-900 flex items-center justify-center gap-4 tracking-tight">
      ...
      <div className="flex flex-col lg:flex-row gap-12 items-start">
        ...
        {/* Left Side: Photo Preview (Live Dynamic) */}
        <div className="w-full lg:w-1/3 flex flex-col items-center p-8 bg-indigo-50/50 rounded-3xl border border-indigo-100">
          <div className="relative group">
            ...
            <img
              // এখানে photoURL স্টেট ব্যবহার করা হয়েছে যাতে ইনপুট পরিবর্তনের সাথে সাথে ছবি বদলে যায়
              src={photoURL || "https://i.ibb.co/9yK7qfM/user.png"}
              alt="Profile Preview"
              className="relative w-44 h-44 md:w-56 md:h-56 rounded-full border-[6px] border-white shadow-2xl object-cover z-10 transition-all duration-500 group-hover:scale-105"
              onError={(e) => { e.target.src = "https://i.ibb.co/9yK7qfM/user.png" }}
            />
            {userData?.isPremium && (
              <div className="absolute top-2 right-2 z-20 bg-gradient-to-tr from-amber-400 to-yellow-600 p-3 rounded-full shadow-lg border-2 border-white animate-bounce">
                <Sparkles className="w-5 h-5 text-white fill-white" />
              </div>
            )}
            <div className="absolute bottom-4 right-4 z-20 bg-indigo-600 p-2.5 rounded-full border-4 border-white text-white shadow-lg">
              <Camera className="w-5 h-5" />
            </div>
          </div>
          
          <div className="mt-6 text-center">
             <h3 className="text-2xl font-black text-gray-800 uppercase tracking-wide">{name || "Your Name"}</h3>
             <p className="text-indigo-600 font-medium">{currentUser?.email}</p>
          </div>
        </div>

        {/* Right Side: Dynamic Form (Same as Register Style) */}
        <div className="flex-1 w-full space-y-8">
          <form onSubmit={handleSubmit} className="space-y-6">
            
            {/* Display Name Input */}
            <div className="relative group">
              <label className="block text-sm font-bold text-indigo-900 uppercase tracking-widest mb-2 ml-1">Full Name</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-14 pr-6 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:bg-white transition-all duration-300"
                  placeholder="Enter your name"
                />
              </div>
            </div>

            {/* Avatar URL Input (Register Style) */}
            <div className="relative group">
              <label className="block text-sm font-bold text-indigo-900 uppercase tracking-widest mb-2 ml-1">Avatar URL (Live Preview)</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-14 pr-6 py-4 bg-gray-50 border border-gray-200 rounded-2xl text-gray-800 focus:outline-none focus:ring-2 focus:ring-purple-500/50 focus:bg-white transition-all duration-300"
                  placeholder="https://example.com/your-image.jpg"
                />
              </div>
              <p className="text-[10px] text-gray-400 mt-2 ml-1">Paste a direct image link to update your avatar instantly above.</p>
            </div>
            ...
            >
              ...
              {loading ? (
                ...
                <>
                  <span className="uppercase tracking-widest">Secure Update</span>
      ...
      {/* Stats Grid Section (Unchanged logic, just cleaner UI) */}
      ...
      {/* Public Lessons Section - Same logic with clean grid */}
      <div className="mt-24">
