  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-10 overflow-hidden">
      ...
      <div className="text-center mb-16" data-aos="fade-down">
        ...
        <h2 className="text-5xl md:text-6xl font-black text-base-content tracking-tighter">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-4" data-aos="fade-right">
          <div className="sticky top-10 bg-base-100 border border-base-200 p-8 rounded-[3.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.05)] flex flex-col items-center text-center group">
            ...
            <div className="mt-8 space-y-2">
              <h3 className="text-3xl font-black text-base-content tracking-tight">{name || "Your Name"}</h3>
              <p className="text-base-content/70 font-medium text-sm truncate max-w-[200px]">{currentUser?.email}</p>
              ...
              <div className="pt-4">
                 {userData?.isPremium ? (
                   ...
                   <div className="px-6 py-2 bg-base-200 text-base-content/70 text-[10px] font-black uppercase rounded-full">
                     Standard Member
                   </div>
                 )}
              </div>
        ...
        <div className="lg:col-span-8" data-aos="fade-left">
          <div className="bg-base-100 border border-base-200 p-8 md:p-12 rounded-[3.5rem] shadow-inner">
            <form onSubmit={handleSubmit} className="space-y-8">
              <div className="grid grid-cols-1 gap-6">
                
                {/* Name Input */}
                <div className="space-y-3">
                  <label className="text-[11px] font-black uppercase tracking-[0.2em] text-base-content/70 ml-4">Full Name</label>
                  <div className={`relative transition-all duration-300 ${isFocused === 'name' ? 'scale-[1.02]' : ''}`}>
                    <User className={`absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 transition-colors ${isFocused === 'name' ? 'text-indigo-600' : 'text-base-content/40'}`} />
                    <input
                      type="text"
                      value={name}
                      ...
                      className="w-full pl-16 pr-8 py-5 bg-base-200 border-none rounded-3xl shadow-sm focus:ring-4 focus:ring-indigo-500/10 outline-none font-bold text-base-content"
                ...
                <div className="space-y-3">
                  <label className="text-[11px] font-black uppercase tracking-[0.2em] text-base-content/70 ml-4">Avatar Image URL</label>
                  <div className={`relative transition-all duration-300 ${isFocused === 'photo' ? 'scale-[1.02]' : ''}`}>
                    <LinkIcon className={`absolute left-6 top-1/2 -translate-y-1/2 w-5 h-5 transition-colors ${isFocused === 'photo' ? 'text-purple-600' : 'text-base-content/40'}`} />
                    <input
                      type="url"
                      value={photoURL}
                      ...
                      className="w-full pl-16 pr-8 py-5 bg-base-200 border-none rounded-3xl shadow-sm focus:ring-4 focus:ring-purple-500/10 outline-none font-bold text-base-content"
                      placeholder="https://images.com/my-photo.jpg"
                    />
                  </div>
                  <p className="text-[10px] text-base-content/70 ml-4 italic">* Paste an image link to see live preview on the left</p>
                </div>
              </div>
...
const StatCard = ({ count, label, icon, color }) => {
  ...
  return (
    <div className="bg-base-100 border border-base-200 p-8 rounded-[2.5rem] shadow-sm flex flex-col items-center group hover:shadow-xl transition-all duration-500">
      <div className={`p-4 rounded-2xl ${colors[color]} mb-4 transition-transform group-hover:scale-110 group-hover:rotate-12`}>
        {React.cloneElement(icon, { size: 24 })}
      </div>
      <span className="text-4xl font-black text-base-content tracking-tighter">{count}</span>
      <span className="text-[10px] font-black uppercase text-base-content/70 tracking-widest mt-1">{label}</span>
    </div>
  );
};
