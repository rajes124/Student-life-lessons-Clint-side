...
const MyLessons = () => {
  ...
  const handleDelete = (id, title) => {
    toast(
      (t) => (
        <div className="text-center p-6 bg-white rounded-3xl shadow-2xl max-w-[90vw] md:max-w-sm mx-auto border border-red-50">
          ...
          <p className="font-black text-xl md:text-2xl mb-2 text-slate-900">Delete Lesson?</p>
          <p className="text-slate-500 mb-6 italic text-sm md:text-base line-clamp-2">"{title}"</p>
          <div className="flex justify-center gap-3">
            ...
            <button
              ...
              className="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-2xl font-bold transition-all text-sm md:text-base"
            >
              Cancel
            </button>
          </div>
  ...
  if (loading) {
    return (
      <div className="min-h-[80vh] flex flex-col items-center justify-center space-y-4 px-4">
        ...
        <p className="text-sm md:text-xl font-black text-slate-400 tracking-tighter uppercase text-center">Syncing Insights...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-[70vh] flex items-center justify-center p-4">
        <div className="text-center group w-full max-w-md">
          ...
          <h2 className="text-3xl md:text-4xl font-black text-slate-800 mb-4">No Wisdom Shared</h2>
          <p className="text-slate-500 mb-8 px-4 font-bold text-sm md:text-base">Your digital archive is waiting for its first spark of wisdom.</p>
          <Link
    ...
    <div className="max-w-7xl mx-auto px-4 py-6 md:py-10 md:px-8">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8 md:mb-12">
        <div className="text-center md:text-left">
          <h2 className="text-3xl md:text-6xl font-black text-slate-900 tracking-tight flex items-center justify-center md:justify-start gap-4">
            My <span className="text-indigo-600 underline decoration-indigo-100 underline-offset-8">Lessons</span>
          </h2>
          <p className="text-slate-500 font-bold mt-4 flex items-center justify-center md:justify-start gap-2 text-xs md:text-sm uppercase tracking-widest">
          ...
          <table className="w-full text-left border-separate border-spacing-0">
            <thead>
              <tr className="bg-slate-50/80">
                <th className="p-4 md:p-6 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100">Lesson & Details</th>
                <th className="p-6 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100 hidden lg:table-cell">Category</th>
                <th className="p-4 md:p-6 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100">Status & Access</th>
                <th className="p-6 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100 hidden md:table-cell text-center">Stats</th>
                <th className="p-4 md:p-6 text-[10px] font-black uppercase tracking-widest text-slate-400 border-b border-slate-100 text-right">Actions</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {lessons.map((lesson) => (
                <tr key={lesson._id} className="group hover:bg-indigo-50/40 transition-all">
                  ...
                  <td className="p-4 md:p-6">
                    <div className="flex items-center gap-3 md:gap-5">
                      <div className="relative flex-shrink-0">
                        {lesson.imageURL ? (
                          <img src={lesson.imageURL} alt="" className="w-12 h-12 md:w-16 md:h-16 object-cover rounded-xl md:rounded-2xl shadow-md border-2 border-white ring-1 ring-slate-100" />
                        ) : (
                          <div className="w-12 h-12 md:w-16 md:h-16 bg-slate-100 rounded-xl md:rounded-2xl flex items-center justify-center">
                            <BookOpen className="w-5 h-5 md:w-6 md:h-6 text-slate-300" />
                          </div>
                        )}
                      </div>
                      <div className="min-w-0 max-w-[120px] sm:max-w-xs lg:max-w-md">
                        <h4 className="font-black text-slate-900 truncate text-sm md:text-lg group-hover:text-indigo-600 transition-colors uppercase tracking-tight">{lesson.title}</h4>
                  ...
                  <td className="p-6 hidden lg:table-cell">
                    <div className="flex flex-col gap-1">
                      ...
                      <p className="text-[11px] text-slate-400 font-bold italic pl-1">Tone: {lesson.emotionalTone}</p>
                    </div>
                  </td>
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="relative group/select">
                        <select
                          ...
                          className={`w-full text-[9px] md:text-[10px] font-black uppercase px-2 md:px-3 py-1.5 md:py-2 rounded-lg border-2 border-transparent transition-all cursor-pointer outline-none ${lesson.visibility === 'public' ? 'bg-green-50 text-green-600 border-green-100' : 'bg-slate-100 text-slate-500'}`}
                        >
                  ...
                  <td className="p-6 hidden md:table-cell">
                    <div className="flex justify-center items-center gap-6">
                      <div className="text-center group/stat">
                        ...
                        <p className="text-[9px] uppercase font-black text-slate-300 tracking-widest">Appreciation</p>
                      </div>
                      <div className="text-center group/stat">
                        ...
                        <p className="text-[9px] uppercase font-black text-slate-300 tracking-widest">Collections</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="p-4 md:p-6 text-right">
                    <div className="flex items-center justify-end gap-1.5 md:gap-3">
                      <Link to={`/lessons/${lesson._id}`} className="p-2 md:p-3 bg-slate-50 text-slate-400 rounded-xl hover:bg-indigo-600 hover:text-white transition-all shadow-sm" title="View"><Eye className="w-4 h-4 md:w-5 md:h-5" /></Link>
                      <Link to={`/dashboard/update-lesson/${lesson._id}`} className="p-2 md:p-3 bg-slate-50 text-slate-400 rounded-xl hover:bg-amber-500 hover:text-white transition-all shadow-sm" title="Edit"><Edit className="w-4 h-4 md:w-5 md:h-5" /></Link>
                      <button onClick={() => handleDelete(lesson._id, lesson.title)} className="p-2 md:p-3 bg-slate-50 text-slate-400 rounded-xl hover:bg-red-600 hover:text-white transition-all shadow-sm" title="Delete"><Trash2 className="w-4 h-4 md:w-5 md:h-5" /></button>
                    </div>
                  </td>
      ...
      <div className="mt-8 md:mt-12 flex flex-col sm:flex-row items-center justify-between text-slate-400 text-[10px] md:text-xs font-black uppercase tracking-[0.2em] px-4 md:px-8 gap-4">
