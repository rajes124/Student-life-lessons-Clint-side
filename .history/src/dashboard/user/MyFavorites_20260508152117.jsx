    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-base-100">
        <div className="text-center">
          ...
          <p className="text-3xl font-bold text-base-content">Loading your favorites...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="max-w-2xl mx-auto mb-12">
          <div className="relative">
            <Search className="absolute left-5 top-1/2 transform -translate-y-1/2 text-base-content/40 w-7 h-7" />
            <input
              type="text"
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              placeholder="Search in your favorites..."
              className="w-full pl-16 pr-8 py-5 text-lg border-0 rounded-2xl shadow-xl focus:ring-4 focus:ring-pink-300 focus:outline-none bg-base-100 placeholder:text-base-content/40 transition-all"
            />
          </div>
        </div>
        ...
        {filteredFavorites.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-2xl text-base-content/70">No lessons found matching your search.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
            {filteredFavorites.map((lesson, index) => {
              ...
              return (
                <div
                  key={lesson._id}
                  className="group relative bg-base-100 rounded-3xl shadow-2xl overflow-hidden transition-all duration-500 hover:shadow-3xl hover:-translate-y-6 border border-base-200 hover:border-pink-300 flex flex-col"
                ...
                >
                  ...
                  <div className="p-8 flex-grow">
                    <h3 className="text-2xl font-bold text-base-content mb-4 line-clamp-2 group-hover:text-indigo-600 transition-colors">
                      {lesson.title}
                    </h3>
                  ...
                  <div className="sticky bottom-0 left-0 right-0 bg-base-100 border-t border-base-200 p-5 flex justify-between items-center gap-4 z-10 shadow-inner">
                    <Link
