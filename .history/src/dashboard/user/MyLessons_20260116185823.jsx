        ...
        <table className="w-full border-collapse bg-white min-w-max">
          <thead>
            <tr className="bg-gradient-to-r from-indigo-600 to-purple-700 text-white">
              <th className="p-5 text-left text-lg font-bold">Lesson</th>
              <th className="p-5 text-left text-lg font-bold">Category</th>
              <th className="p-5 text-left text-lg font-bold">Tone</th>
              <th className="p-5 text-left text-lg font-bold">Visibility</th>
              <th className="p-5 text-left text-lg font-bold">Access</th>
              <th className="p-5 text-left text-lg font-bold">Created</th>
              <th className="p-5 text-left text-lg font-bold">Stats</th>
              <th className="p-5 text-center text-lg font-bold">Actions</th>
            </tr>
          </thead>
          <tbody>
            {lessons.map((lesson) => (
              <tr
                key={lesson._id}
                className="border-b hover:bg-indigo-50/50 transition-all duration-200"
              >
                <td className="p-5">
                  <div className="flex items-center gap-4">
                    {lesson.imageURL ? (
                      <img
                        src={lesson.imageURL}
                        alt={lesson.title}
                        className="w-16 h-16 object-cover rounded-lg shadow-md ring-2 ring-indigo-100"
                      />
                    ) : (
                ...
                <td className="p-5 text-gray-700">{lesson.category}</td>
                <td className="p-5 text-gray-700">{lesson.emotionalTone}</td>

                <td className="p-5">
                  <select
                    value={lesson.visibility}
                    onChange={(e) => handleVisibilityChange(lesson._id, e.target.value)}
                    className="w-full px-4 py-2.5 border rounded-lg font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all"
                  >
                ...
                <td className="p-5 text-center flex items-center justify-center gap-6">
                  <Link
